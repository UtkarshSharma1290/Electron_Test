...
class Recording {
  constructor(options = {}) {
    ...
    this.cmd = cmd;
    this.args = args; // Default args for system output capture
  ...
  // Method to switch recording to capture from a specific input device
  switchInputDevice(deviceName) {
    console.log({ deviceName });
    // Modify args based on deviceName and restart recording
    this.args = [
      "-t",
      "coreaudio",
      deviceName,
  ...
  // Method to restart recording with updated args
  restart() {
    this.stop();
    return this.start();
  }

...
export default function Dashboard() {
  ...
  /**
   * Function for toggling between pause and play
   * @returns
   */
  const onTogglePause = () => {
    if (isRecordingPause) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (inputText === "") {
      setRecordingIcon(false);
      updateUserResponse({
        userTranscript: openMic ? `${chatLog?.length + 1}:${textToShow}` : "",
    ...
    try {
      ...
      //TODO: have to call an api to update the users mic audio transcription

      if (chatLog?.length >= 1) {
        updateUserResponse({
          userTranscript: `${chatLog?.length}:${textToShow || ""}`,
          sessionId: currentSelectedThread,
        });
      }
                ...
                <div className="w-full h-full max-h-[calc(100%_-_380px)] py-4 pt-0 flex items-star flex-col gap-2">
                  <Sidebar
                    ...
                    sessionData={
                      ...
                      selectedTags?.length ? userSearchResult : allSessionData
                    }

    ...
    if (window.require) {
      ...
      ipcRenderer.on("external-url-opened", (event, args) => {
        ...
        addAuthenticateUserToDb({
          email: window.localStorage.getItem(USER_EMAIL),
          onErrorCallback: () => {
            toast?.error("Login has failed please try again in a while!");
            logout(() => navigate(LOGIN));
            console.log("calling login re-direct....");
          },
          onSuccessCallback: () => {
            console.log("getting called success....");
            localStorage.setItem("verified", true);
            console.log({ email });
            // window.localStorage.setItem(USER_EMAIL, email);
            navigate("/dependency");
